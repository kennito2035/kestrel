 ...
 * bottom source pixel pairs) and one vertical. We map the two horizontal
 * lerps onto INTERP0 and INTERP1 and keep the vertical lerp in software,
...
void resize_bilinear_interp(const uint8_t *src, uint16_t src_w, uint16_t src_h,
                            uint8_t *dst)
{
    /* Both interpolators in blend mode: INTERP0 lerps the top pixel pair,
     * INTERP1 the bottom pair, sharing the same X alpha. */
    ...
    for (uint16_t dy = 0; dy < KESTREL_DST_H; dy++, y_fp += y_step) {
        ...
        for (uint16_t dx = 0; dx < KESTREL_DST_W; dx++, x_fp += x_step) {
            ...
            interp0->accum[1] = fx;
            interp1->base[0] = row1[sx];
            interp1->base[1] = row1[sx1];
            interp1->accum[1] = fx;

            const uint8_t top = (uint8_t)interp0->peek[1];
            const uint8_t bot = (uint8_t)interp1->peek[1];
            dst[(uint32_t)dy * KESTREL_DST_W + dx] = blend8(top, bot, fy);
        }
    }
}
