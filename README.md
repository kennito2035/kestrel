...
│
├── stm32h750/
│   ├── firmware/08-DCMI2LCD/       # ← The complete shipping firmware (CubeIDE project)
│   │   ├── Src/                    #    main.c pipeline (gate, ROI attention, HUD,
│   │   │                           #    STOP/wake), detection.c, img_preproc.c,
│   │   │                           #    ai_infer.c, gate.c, uart_log.c, stop_mode.c,
│   │   │                           #    temp_sensor.c + X-CUBE-AI network*.c
│   │   ├── Inc/ (headers)
│   │   ├── Drivers/BSP/            #    WeAct OV2640 camera + ST7735 LCD drivers
│   │   ├── STM32H750VBTX_QSPI.ld   #    QSPI-XIP link script (bootloader-aware)
│   │   └── 08-DCMI2LCD.ioc         #    CubeIDE/CubeMX project config
│   ├── Core/                       # Standalone DWT benchmark harness scaffold
│   └── Drivers/ov3660/             # Experimental OV3660 DCMI driver (standalone)
│
├── rp2350/
│   ├── src/
...
│   ├── arduino/kestrel_rp2350/     # Arduino IDE cascade firmware (USB telemetry echo)
│   ├── arduino/kestrel_interp_bench/ # Arduino IDE bench (produced the published numbers)
│   └── CMakeLists.txt
│
├── training/                       # Model selection + optional retrain path
├── benchmarks/                     # benchmark_report.md, interpolator_results.csv,
│                                   #   filmed skip-rate runs, summarize.py
├── docs/
